 ...
 *  ──────────────────────────────────────────────────────────────────────
 *  Содержит тот же алгоритм, что и исходная программа, но «fuzz‑friendly».
 *
 *  * При запуске не принимаются аргументы командной строки – вместо этого
 *    программа читает два целых числа из stdin.
 *  * Это делает её удобной для AFL++: каждый тест‑файл будет
 *    интерпретироваться как «operation value».
 *  * Прочие части кода (ссылочный список, утечки, рекурсия) оставлены без
 *    изменений, чтобы сохранять поведение оригинала.
 *
 *  Компиляция с AFL++:
 *
 *      afl-clang-fast -O1 -g \
...
#include <stdlib.h>
#include <string.h>
#include <pthread.h>      /* нужен для кеш‑модуля в main */
...
static CacheEntry *global_cache = NULL;   /* упрощённый кеш из примера 2 */
...
int main(void) {
    ...
    if (!my_list) return 1;   /* не критично – но даём понятный код выхода */
